...
export default function CandidateLogin() {
  ...
  const handleLogin = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const candidateResponse = await fetch("http://127.0.0.1:8000/api/candidate/get_candidate", {
        method: "POST",
      ...
      alert("Server error while fetching candidate details.");
      return;
    }

    try {
      // ✅ Step 2: Verify Password
      const passwordResponse = await fetch("http://127.0.0.1:8000/api/candidate/verify-password", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    try {
      ...
      const faceResponse = await fetch("http://127.0.0.1:8000/api/candidate/recognize", {
        method: "POST",
        body: formData,
      });
      ...
      if (recognitionResult.status === "success") {
        if (recognitionResult.recognized_user.universityID === loginData.universityID) {
          ...
          navigate("/candidate_dashboard", { state: { candidate } });
      ...
      <div className="card p-4 shadow w-50 text-center">
        ...
        <form onSubmit={handleLogin}>
          ...
          <div className="webcam-container position-relative mb-3">
            ...
            {recognizedUser && faceBox?.length === 4 && (
              <div 
          ...
          {isVerifying && (
            <p className="text-warning">
              <span className="spinner-border spinner-border-sm"></span> Verifying face, please wait...
            </p>
          )}

          <button type="submit" className="btn btn-primary w-100 mt-3" disabled={isVerifying}>
            {isVerifying ? "Processing..." : "Login"}
          </button>
        </form>

        <p className="mt-2">
          New User? <Link to="/candidate_register"> Sign Up</Link>
        </p>
      </div>
    </div>
  );
